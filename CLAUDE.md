...
- **Streaming test** — sidebar `Test streaming` toggle (disabled when mock tools on). Instruments the normal stream path with `stream_verdict` and `stream_trace_summary` stored on `stats` for the assistant message; verdict line shown in **Details** on replay.
