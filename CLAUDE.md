...
- **Mock skills probe** — sidebar `Enable mock skills` with two modes: **Catalog + load_skill** (skill catalog appended to system; `load_skill` tool returns bundled SKILL.md) or **Full skill in system** (entire SKILL.md in system; no `load_skill`). Uses the same `run_tool_chat` loop as mock tools. **Skills trace** expander shows mode, system excerpt, and `load_skill_calls`; full rounds in **Tool trace**. `Send test prompt` sends `Say ping.`
